use crate::{
    ...
    state::ayclend_group::{Bank, BankVaultType},
    AyclendError, AyclendResult,
};
...
pub fn nonzero_fee(mint_ai: AccountInfo, epoch: u64) -> AyclendResult<bool> {
    if mint_ai.owner.eq(&Token::id()) {
        return Ok(false);
    }
...
) -> AyclendResult<Option<InterfaceAccount<'info, Mint>>> {
    match *token_program {
        anchor_spl::token::ID => Ok(None),
        anchor_spl::token_2022::ID => {
            let (maybe_mint, remaining) = remaining_accounts
                .split_first()
                .ok_or(AyclendError::T22MintRequired)?;
            *remaining_accounts = remaining;

            if bank.mint != *maybe_mint.key {
                return err!(AyclendError::T22MintRequired);
            }
            ...
            InterfaceAccount::try_from(maybe_mint)
                ...
                .map_err(|e| {
                    ...
                    AyclendError::T22MintRequired.into()
                })
        }
