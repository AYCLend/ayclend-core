use super::{
    ayclend_account::{BalanceSide, RequirementType},
...
use crate::{
    ...
    debug, math_error,
    prelude::AyclendError,
    set_if_some,
    state::ayclend_account::calc_value,
    AyclendResult,
};
...
impl AyclendGroup {
    ...
    pub fn configure(&mut self, config: &GroupConfig) -> AyclendResult {
...
pub fn load_pyth_price_feed(ai: &AccountInfo) -> AyclendResult<PriceFeed> {
...
impl InterestRateConfig {
    ...
    pub fn validate(&self) -> AyclendResult {
        ...
        check!(
            ...
            AyclendError::InvalidConfig
        );
        check!(plateau_ir > I80F48::ZERO, AyclendError::InvalidConfig);
        check!(max_ir > I80F48::ZERO, AyclendError::InvalidConfig);
        check!(plateau_ir < max_ir, AyclendError::InvalidConfig);
...
impl Bank {
    ...
    pub fn get_liability_amount(&self, shares: I80F48) -> AyclendResult<I80F48> {
        Ok(shares
            .checked_mul(self.liability_share_value.into())
            .ok_or_else(math_error!())?)
    }

    pub fn get_asset_amount(&self, shares: I80F48) -> AyclendResult<I80F48> {
        Ok(shares
            .checked_mul(self.asset_share_value.into())
            .ok_or_else(math_error!())?)
    }

    pub fn get_liability_shares(&self, value: I80F48) -> AyclendResult<I80F48> {
        Ok(value
            .checked_div(self.liability_share_value.into())
            .ok_or_else(math_error!())?)
    }

    pub fn get_asset_shares(&self, value: I80F48) -> AyclendResult<I80F48> {
        Ok(value
    ...
    pub fn change_asset_shares(
        ...
        if shares.is_positive() && self.config.is_deposit_limit_active() && !bypass_deposit_limit {
            ...
            check!(
                ...
                crate::prelude::AyclendError::BankAssetCapacityExceeded
            )
        }
    ...
    pub fn change_liability_shares(
        ...
        if bypass_borrow_limit.not() && shares.is_positive() && self.config.is_borrow_limit_active()
        {
            ...
            check!(
                ...
                crate::prelude::AyclendError::BankLiabilityCapacityExceeded
            )
        }
    ...
    pub fn check_utilization_ratio(&self) -> AyclendResult {
        ...
        check!(
            ...
            crate::prelude::AyclendError::IllegalUtilizationRatio
        );

        Ok(())
    }

    pub fn configure(&mut self, config: &BankConfigOpt) -> AyclendResult {
    ...
    pub fn socialize_loss(&mut self, loss_amount: I80F48) -> AyclendResult {
        let total_asset_shares: I80F48 = self.total_asset_shares.into();
    ...
    pub fn assert_operational_mode(
        ...
        match self.config.operational_state {
            BankOperationalState::Paused => Err(AyclendError::BankPaused.into()),
...
impl BankConfig {
    ...
    pub fn validate(&self) -> AyclendResult {
        ...
        check!(asset_maint_w >= asset_init_w, AyclendError::InvalidConfig);

        let liab_init_w = I80F48::from(self.liability_weight_init);
        let liab_maint_w = I80F48::from(self.liability_weight_maint);

        check!(liab_init_w >= I80F48::ONE, AyclendError::InvalidConfig);
        check!(
        ...
        if self.risk_tier == RiskTier::Isolated {
            check!(asset_init_w == I80F48::ZERO, AyclendError::InvalidConfig);
            check!(asset_maint_w == I80F48::ZERO, AyclendError::InvalidConfig);
        }
    ...
    pub fn validate_oracle_setup(&self, ais: &[AccountInfo]) -> AyclendResult {
