use super::{
    ayclend_group::{Bank, RiskTier, WrappedI80F48},
...
assert_struct_size!(AyclendAccount, 2304);
assert_struct_align!(AyclendAccount, 8);
...
impl AyclendAccount {
    /// Set the initial data for the ayclend account.
    ...
    pub fn set_new_account_authority_checked(&mut self, new_authority: Pubkey) -> AyclendResult {
        // check if new account authority flag is set
        if !self.get_flag(TRANSFER_AUTHORITY_ALLOWED_FLAG) || self.get_flag(DISABLED_FLAG) {
            return Err(AyclendError::IllegalAccountAuthorityTransfer.into());
        }
...
pub struct BankAccountWithPriceFeed<'a, 'info> {
    ...
    price_feed: Box<AyclendResult<OraclePriceFeedAdapter>>,
    balance: &'a Balance,
}
...
impl<'info> BankAccountWithPriceFeed<'_, 'info> {
    pub fn load<'a>(
    ...
    ) -> AyclendResult<Vec<BankAccountWithPriceFeed<'a, 'info>>> {
        ...
        check!(
            ...
            AyclendError::MissingPythOrBankAccount
        );
        ...
        active_balances
            ...
            .map(|(i, balance)| {
                ...
                check!(
                    ...
                    AyclendError::InvalidBankAccount
                );
...
impl From<PriceFeedError> for Error {
    fn from(value: PriceFeedError) -> Self {
        match value {
            PriceFeedError::StaleOracle => error!(AyclendError::StaleOracle),
        }
    }
}
...
#[inline]
pub fn calc_amount(value: I80F48, price: I80F48, mint_decimals: u8) -> AyclendResult<I80F48> {
    let scaling_factor = EXP_10_I80F48[mint_decimals as usize];
...
pub struct RiskEngine<'a, 'info> {
    ayclend_account: &'a AyclendAccount,
...
impl<'info> RiskEngine<'_, 'info> {
    pub fn new<'a>(
        ayclend_account: &'a AyclendAccount,
        remaining_ais: &'info [AccountInfo<'info>],
    ) -> AyclendResult<RiskEngine<'a, 'info>> {
        check!(
            !ayclend_account.get_flag(IN_FLASHLOAN_FLAG),
            AyclendError::AccountInFlashloan
        );

        Self::new_no_flashloan_check(ayclend_account, remaining_ais)
    }
    ...
    fn new_no_flashloan_check<'a>(
        ...
        let bank_accounts_with_price =
            BankAccountWithPriceFeed::load(&ayclend_account.lending_account, remaining_ais)?;
    ...
    pub fn check_account_init_health<'a>(
        ...
        if ayclend_account.get_flag(IN_FLASHLOAN_FLAG) {
            return Ok(());
        }
    ...
    fn check_account_health(&self, requirement_type: RiskRequirementType) -> AyclendResult {
        let (total_weighted_assets, total_weighted_liabilities) =
            self.get_account_health_components(requirement_type)?;
        ...
        check!(
            total_weighted_assets >= total_weighted_liabilities,
            AyclendError::RiskEngineInitRejected
        );
    ...
    pub fn check_pre_liquidation_condition_and_get_account_health(
        ...
        check!(
            !self.ayclend_account.get_flag(IN_FLASHLOAN_FLAG),
            AyclendError::AccountInFlashloan
        );
        ...
        let liability_bank_balance = self
            ...
            .ok_or(AyclendError::LendingAccountBalanceNotFound)?;

        check!(
            liability_bank_balance
                .is_empty(BalanceSide::Liabilities)
                .not(),
            AyclendError::IllegalLiquidation
        );
    ...
    pub fn check_account_bankrupt(&self) -> AyclendResult {
        ...
        check!(
            ...
            AyclendError::AccountNotBankrupt
        );
        check!(
    ...
    fn check_account_risk_tiers<'a>(&'a self) -> AyclendResult
    where
        'info: 'a,
    {
        ...
        check!(
            ...
            AyclendError::IsolatedAccountIllegalState
        );
...
impl Balance {
    ...
    pub fn change_asset_shares(&mut self, delta: I80F48) -> AyclendResult {
    ...
    pub fn change_liability_shares(&mut self, delta: I80F48) -> AyclendResult {
    ...
    pub fn close(&mut self) -> AyclendResult {
        check!(
            I80F48::from(self.emissions_outstanding) < I80F48::ONE,
            AyclendError::CannotCloseOutstandingEmissions
        );
...
impl<'a> BankAccountWrapper<'a> {
    ...
    ) -> AyclendResult<BankAccountWrapper<'a>> {
        let balance = lending_account
            .balances
            .iter_mut()
            .find(|balance| balance.active && balance.bank_pk.eq(bank_pk))
            .ok_or_else(|| error!(AyclendError::BankAccountNotFound))?;
        ...
        match balance_index {
            ...
            None => {
                let empty_index = lending_account
                    ...
                    .ok_or_else(|| error!(AyclendError::LendingAccountBalanceSlotsFull))?;
    ...
    pub fn deposit(&mut self, amount: I80F48) -> AyclendResult {
    ...
    pub fn repay(&mut self, amount: I80F48) -> AyclendResult {
    ...
    pub fn increase_balance(&mut self, amount: I80F48) -> AyclendResult {
        self.increase_balance_internal(amount, BalanceIncreaseType::Any)
    }

    pub fn increase_balance_in_liquidation(&mut self, amount: I80F48) -> AyclendResult {
    ...
    /// the specified deposit amount and the existing balance.
    pub fn decrease_balance(&mut self, amount: I80F48) -> AyclendResult {
        self.decrease_balance_internal(amount, BalanceDecreaseType::Any)
    }
    ...
    pub fn decrease_balance_in_liquidation(&mut self, amount: I80F48) -> AyclendResult {
    ...
    pub fn withdraw_all(&mut self) -> AyclendResult<u64> {
    ...
    pub fn repay_all(&mut self) -> AyclendResult<u64> {
        self.claim_emissions(Clock::get()?.unix_timestamp as u64)?;
    ...
    pub fn close_balance(&mut self) -> AyclendResult<()> {
        self.claim_emissions(Clock::get()?.unix_timestamp as u64)?;
        ...
        check!(
            current_liability_amount.is_zero_with_tolerance(ZERO_AMOUNT_THRESHOLD),
            AyclendError::IllegalBalanceState,
    ...
    pub fn claim_emissions(&mut self, current_timestamp: u64) -> AyclendResult {
    ...
    pub fn settle_emissions_and_get_transfer_amount(&mut self) -> AyclendResult<u64> {
        self.claim_emissions(Clock::get()?.unix_timestamp as u64)?;
