...
use super::ayclend_group::BankConfig;
...
impl OraclePriceFeedAdapter {
    ...
    pub fn try_from_bank_config_with_max_age(
        ...
        match bank_config.oracle_setup {
            OracleSetup::None => Err(AyclendError::OracleNotSetup.into()),
            OracleSetup::PythLegacy => {
                check!(ais.len() == 1, AyclendError::InvalidOracleAccount);
                check!(
                    ais[0].key == &bank_config.oracle_keys[0],
                    AyclendError::InvalidOracleAccount
                );
            ...
            OracleSetup::CustomA => {
                ...
                Ok(OraclePriceFeedAdapter::CustomA(
                    CustomAPriceFeed::load_checked(&ais[0], clock.unix_timestamp, max_age)?,
                ))
            },
        }
    }
    ...
    pub fn validate_bank_config(
        ...
        match bank_config.oracle_setup {
            OracleSetup::None => Err(AyclendError::OracleNotSetup.into()),
            OracleSetup::PythLegacy => {
                check!(oracle_ais.len() == 1, AyclendError::InvalidOracleAccount);
                check!(
            ...
            OracleSetup::CustomA => {
                ...
                CustomAPriceFeed::check_ais(&oracle_ais[0])?;
                Ok(())
            }
        }
    }
...
impl PythLegacyPriceFeed {
    pub fn load_checked(ai: &AccountInfo, current_time: i64, max_age: u64) -> AyclendResult<Self> {
        let price_feed = load_pyth_price_feed(ai)?;

        let ema_price = price_feed
            .get_ema_price_no_older_than(current_time, max_age)
            .ok_or(AyclendError::StaleOracle)?;
    ...
    fn check_ais(ai: &AccountInfo) -> AyclendResult {
        load_pyth_price_feed(ai)?;
        Ok(())
    }

    fn get_confidence_interval(&self, use_ema: bool) -> AyclendResult<I80F48> {
    ...
    #[inline(always)]
    fn get_ema_price(&self) -> AyclendResult<I80F48> {
        pyth_price_components_to_i80f48(I80F48::from_num(self.ema_price.price), self.ema_price.expo)
    }

    #[inline(always)]
    fn get_unweighted_price(&self) -> AyclendResult<I80F48> {
...
impl SwitchboardV2PriceFeed {
    pub fn load_checked(
        ...
        let aggregator_account = AggregatorAccountData::new_from_bytes(&ai_data)
            .map_err(|_| AyclendError::InvalidOracleAccount)?;

        aggregator_account
            .check_staleness(current_timestamp, max_age as i64)
            .map_err(|_| AyclendError::StaleOracle)?;
    ...
    fn get_price(&self) -> AyclendResult<I80F48> {
        let sw_decimal = self
            .aggregator_account
            .get_result()
            .map_err(|_| AyclendError::InvalidPrice)?;

        Ok(switchboard_decimal_to_i80f48(sw_decimal)
            .ok_or(AyclendError::InvalidSwitchboardDecimalConversion)?)
    }

    fn get_confidence_interval(&self) -> AyclendResult<I80F48> {
...
pub fn load_price_update_v2_checked(ai: &AccountInfo) -> AyclendResult<PriceUpdateV2> {
    check!(
...
impl PythPushOraclePriceFeed {
    ...
    /// There can be multiple pyth push oracles for a given feed_id. Ayclend allows using any
            ...
            .map_err(|e| {
                ...
                match e {
                    pyth_solana_receiver_sdk::error::GetPriceError::PriceTooOld => {
                        AyclendError::StaleOracle
                    }
                    _ => AyclendError::InvalidOracleAccount,
                }
            })?;
    ...
    #[cfg(feature = "client")]
    pub fn load_unchecked(ai: &AccountInfo) -> AyclendResult<Self> {
        let price_feed_account = load_price_update_v2_checked(ai)?;
    ...
    #[cfg(feature = "client")]
    pub fn peek_feed_id(ai: &AccountInfo) -> AyclendResult<FeedId> {
        let price_feed_account = load_price_update_v2_checked(ai)?;

        Ok(price_feed_account.price_message.feed_id)
    }

    pub fn check_ai_and_feed_id(ai: &AccountInfo, feed_id: &FeedId) -> AyclendResult {
        let price_feed_account = load_price_update_v2_checked(ai)?;
...
#[cfg_attr(feature = "client", derive(Clone, Debug))]
pub struct CustomAPriceFeed {
  price: u64,
}

impl CustomAPriceFeed {
    pub fn load_checked(
        ai: &AccountInfo,
        current_timestamp: i64,
        max_age: u64,
        ...
        check!(
            ai.owner.eq(&Pubkey::from_str("EpPyE6LL6W5Gge5S83Zv96EaVHwYcC3wiXRDAVFqzjtM").unwrap()),
            AyclendError::InvalidOracleAccount
        );

        let input = array_ref![&ai_data, 0, 32 + 8 + 8];
        let (_owner, price, timestamp) = array_refs![input, 32, 8, 8];

        let timestamp = u64::from_le_bytes(*timestamp);
        msg!("timestamp {}, current {}, max age {}", timestamp, current_timestamp, max_age);
        check!(
           timestamp + max_age > (current_timestamp as u64),
           AyclendError::StaleOracle
        );
...
impl PriceAdapter for CustomAPriceFeed {
    fn get_price_of_type(
        &self,
        _price_type: OraclePriceType,
        _bias: Option<PriceBias>,
    ) -> AyclendResult<I80F48> {
        Ok(I80F48::from_num(self.price).checked_div(EXP_10_I80F48[6 as usize]).unwrap())
	}
}
...
impl LiteAggregatorAccountData {
    ...
    pub fn get_result(&self) -> anchor_lang::Result<SwitchboardDecimal> {
        ...
        if min_oracle_results > latest_confirmed_round_num_success {
            return Err(AyclendError::InvalidOracleAccount.into());
        }
...
#[inline(always)]
fn pyth_price_components_to_i80f48(price: I80F48, exponent: i32) -> AyclendResult<I80F48> {
...
fn load_pyth_price_feed(ai: &AccountInfo) -> AyclendResult<PriceFeed> {
    check!(ai.owner.eq(&PYTH_ID), AyclendError::InvalidOracleAccount);
