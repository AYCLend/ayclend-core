use crate::{
    ...
    state::{
        ...
        ayclend_group::{Bank, BankVaultType, AyclendGroup},
    },
    utils, AyclendResult,
};
...
/// Handle a bankrupt ayclend account.
...
pub fn lending_pool_handle_bankruptcy<'info>(
    ...
    let LendingPoolHandleBankruptcy {
        ...
        token_program,
        bank: bank_loader,
        ayclend_group: ayclend_group_loader,
        ..
    } = ctx.accounts;
    ...
    if !bank.get_flag(PERMISSIONLESS_BAD_DEBT_SETTLEMENT_FLAG) {
        check!(
            ctx.accounts.signer.key() == ayclend_group_loader.load()?.admin,
            AyclendError::Unauthorized
        );
    }
    ...
    RiskEngine::new(&ayclend_account, ctx.remaining_accounts)?.check_account_bankrupt()?;
    ...
    let lending_account_balance = ayclend_account
        .lending_account
        .balances
        .iter_mut()
    ...
    check!(
        ...
        AyclendError::LendingAccountBalanceNotFound
    );
    ...
    check!(
        ...
        AyclendError::BalanceNotBadDebt
    );
    ...
    ayclend_account.set_flag(DISABLED_FLAG);
