...
use crate::state::ayclend_account::{calc_amount, calc_value, RiskEngine};
use crate::state::ayclend_group::{Bank, BankVaultType};
use crate::state::price::{OraclePriceFeedAdapter, OraclePriceType, PriceAdapter, PriceBias};
use crate::{
    bank_signer,
    constants::{LIQUIDITY_VAULT_AUTHORITY_SEED, LIQUIDITY_VAULT_SEED},
    state::ayclend_account::{BankAccountWrapper, AyclendAccount},
};
...
pub fn lending_account_liquidate<'info>(
    ...
    let LendingAccountLiquidate {
        liquidator_ayclend_account: liquidator_ayclend_account_loader,
        liquidatee_ayclend_account: liquidatee_ayclend_account_loader,
        ..
    } = ctx.accounts;

    let mut liquidator_ayclend_account = liquidator_ayclend_account_loader.load_mut()?;
    let mut liquidatee_ayclend_account = liquidatee_ayclend_account_loader.load_mut()?;
    let clock = Clock::get()?;
    ...
    let init_liquidatee_remaining_len = liquidatee_ayclend_account.get_remaining_accounts_len();
    let pre_liquidation_health = {
        ...
        RiskEngine::new(&liquidatee_ayclend_account, liquidatee_remaining_accounts)?
    ...
    let (pre_balances, post_balances) = {
        ...
        let (liquidator_liability_pre_balance, liquidator_liability_post_balance) = {
            let mut bank_account = BankAccountWrapper::find_or_create(
                &ctx.accounts.liab_bank.key(),
                &mut liab_bank,
                &mut liquidator_ayclend_account.lending_account,
            )?;
        ...
        let (liquidatee_asset_pre_balance, liquidatee_asset_post_balance) = {
            let mut bank_account = BankAccountWrapper::find(
                &ctx.accounts.asset_bank.key(),
                &mut asset_bank,
                &mut liquidatee_ayclend_account.lending_account,
            )?;
            ...
            bank_account
                ...
                .map_err(|_| AyclendError::IllegalLiquidation)?;
    ...
    let liquidator_accounts_starting_pos =
        liquidatee_accounts_starting_pos - liquidator_ayclend_account.get_remaining_accounts_len();
    ...
    emit!(LendingAccountLiquidateEvent {
        header: AccountEventHeader {
            ...
            ayclend_account: liquidator_ayclend_account_loader.key(),
            ayclend_account_authority: liquidator_ayclend_account.authority,
            ayclend_group: ctx.accounts.ayclend_group.key(),
        },
        liquidatee_ayclend_account: liquidatee_ayclend_account_loader.key(),
        liquidatee_ayclend_account_authority: liquidatee_ayclend_account.authority,
...
pub struct LendingAccountLiquidate<'info> {
    ...
    #[account(
        mut,
        constraint = asset_bank.load()?.group == ayclend_group.key()
    )]
    pub asset_bank: AccountLoader<'info, Bank>,

    #[account(
        mut,
        constraint = liab_bank.load()?.group == ayclend_group.key()
    )]
    pub liab_bank: AccountLoader<'info, Bank>,

    #[account(
        mut,
        constraint = liquidator_ayclend_account.load()?.group == ayclend_group.key()
    )]
    pub liquidator_ayclend_account: AccountLoader<'info, AyclendAccount>,

    #[account(
        address = liquidator_ayclend_account.load()?.authority
    )]
    pub signer: Signer<'info>,

    #[account(
        mut,
        constraint = liquidatee_ayclend_account.load()?.group == ayclend_group.key()
    )]
    pub liquidatee_ayclend_account: AccountLoader<'info, AyclendAccount>,
